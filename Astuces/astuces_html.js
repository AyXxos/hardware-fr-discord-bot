...
const CACHE_FILE = path.join(__dirname, "../data/caches/astuces_html_cache.json");
const CACHE_DURATION = 90 * 24 * 60 * 60 * 1000;
const LOG_CHANNEL_ID = "1478430424098275348";
const ASTUCES_CHANNEL_ID = "1067905635758374963";
...
function loadCache() {
  ...
  try {
    ...
    console.warn("Cache HTML corrompu, création d'un nouveau cache.");
    return { astuces: [], lastCleanup: Date.now() };
  }
}
...
async function generateHtmlAstuce() {
  ...
  const response = await fetch("https://api.mistral.ai/v1/chat/completions", {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: "system",
          content: "Tu es un expert en développement web.",
        },
        {
          role: "user",
          content: `Donne-moi une astuce HTML peu connue, courte et intéressante, puis explique-la simplement.
...
module.exports = async (bot) => {
  ...
  if (!astucesChannel) {
    ...
    if (logChannel) {
      logChannel.send("❌ Channel astuces HTML introuvable (ID invalide). ");
    }
    return;
  }

  try {
    const { astuce, explication } = await generateHtmlAstuce();
    const embed = new MessageEmbed()
      .setColor(tools.randomColor())
      .setTitle("💡 Astuce HTML")
    ...
    if (logChannel && logChannel.id !== astucesChannel.id) {
      await logChannel.send("✅ Astuce HTML envoyée.");
    }
  } catch (error) {
    console.error("❌ Erreur lors de la génération/envoi de l'astuce HTML:", error.message);
    if (logChannel) {
      await logChannel.send(`❌ Erreur astuce HTML: ${error.message}`);
    }
  }
};